...
{
        int i=0,j=0;
        Align_Params *AP = (Align_Params*) malloc(sizeof(Align_Params));
        (*AP).states = 3;
        (*AP).lTRS = (float **) calloc((*AP).states,sizeof(float*)); // match =0, ins =1, del = 2
        for (i=0;i<AP->states;i++) AP->lTRS[i] = (float*) calloc(sizeof(float),AP->states);
        ...
        AP->lMEM = (float **) calloc(4,sizeof(float*)); 
		for (i=0;i<4;i++) AP->lMEM[i] = (float *) calloc(4,sizeof(float));
        for (i=0;i<4;i++)
        {
                for (j=0;j<4;j++)
                {
        ...
        (*AP).TRS = (float**) calloc((*AP).states,sizeof(float*)); // match =0, ins =1, del = 2
        for (i=0;i<AP->states;i++) AP->TRS[i] = (float *) calloc(sizeof(float),AP->states);
        AP->MEM = (float**) calloc(4,sizeof(float*)); 
		for (i=0;i<4;i++) AP->MEM[i] = (float*) calloc(4,sizeof(float));
...
int realignment_params(char* bamfile,REFLIST* reflist,char* regions,Align_Params* AP)
{
	...
	int i=0;
        ...
        int *indel_lengths = (int*) calloc(40,sizeof(int));
        for (i=0;i<40;i++) indel_lengths[i]=0;
        // 0 = M | I =1, D = 2 from cigar encoding 
	char* newregion = (char*) calloc(4096,sizeof(char));
