...
int cluster_reads(struct alignedread** readlist, int s, int e, FRAGMENT* flist, VARIANT* varlist, REFLIST* reflist) {
    ...
    struct BLOCK_READ* RL = (struct BLOCK_READ*)calloc(sizeof (struct BLOCK_READ), blocks);
    for (i = 0; i < blocks; i++) {
        RL[i].reads = 0;
        RL[i].firstread = -1;
...
int parse_bamfile_fosmid(char* bamfile, HASHTABLE* ht, CHROMVARS* chromvars, VARIANT* varlist, REFLIST* reflist) {
    ...
    struct alignedread** readlist = (struct alignedread**) calloc(MAX_READS, sizeof (struct alignedread*));
    for (reads = 0; reads < MAX_READS; reads++) readlist[reads] = (struct alignedread*) calloc(1, sizeof (struct alignedread));
    ...
    while (sam_read1(fp, header, b) >= 0) {
        ...
        fetch_func(b, fp, header, readlist[r]);
